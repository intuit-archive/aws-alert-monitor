...
describe AwsAlertMonitor::Alert do
  before do
    ...
    @options = {:message=>@message, :events=>{"autoscaling:EC2_INSTANCE_LAUNCH"=>{"email"=>{"source"=>"brett_weaver@intuit.com", "destination"=>"brett_weaver@intuit.com"}}}}
  ...
  it "should process the given message against known events" do
    ...
    ses_mock.should_receive(:send_email)
            .with({:source=>"brett_weaver@intuit.com", :destination=>{:to_addresses=>["brett_weaver@intuit.com"]}, :message=>{:subject=>{:data=>"Launching a new EC2 instance: i-d6a2cb8f"}, :body=>{:text=>{:data=>"At 2012-11-29T16:39:05Z an instance was started in response to a difference between desired and actual capacity, increasing the capacity from 0 to 1."}}}})
